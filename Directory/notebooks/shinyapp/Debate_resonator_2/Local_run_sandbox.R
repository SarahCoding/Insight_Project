...
ui <-fluidPage(
  ...
  mainPanel(
    ...
    leafletOutput("map"),
    p(),
    plotOutput("Plot") # Put the output - a plot - into the main panel
...
barplot(subset(data_scores$norm_cos, data_scores$State=="Mic"))

data_scores(data_scores$State=="Mic")

p<-ggplot(data=data_scores[which(data_scores$State=="Mic"),], aes(x=Candidate, y=norm_cos)) +
...
p=ggplot(data=data_scores, aes(x=Candidate, y=norm_cos, fill=State)) +
  geom_bar(stat="identity", position=position_dodge())+
  geom_text(aes(label=norm_cos), vjust=1.6, color="black",
...
ggplot(data=data_counts, aes(x=Candidate, y=weight, fill=State)) +
  geom_bar(stat="identity", position=position_dodge())+
  geom_text(aes(label=norm_cos), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
  ylab("Similarity Scores (%)")+
